...
import { Setup2FA } from "./auth/Setup2FA" // ✅ AGREGAR ESTA LÍNEA
...
export function Dashboard({ onLogout, currentUser }: DashboardProps) {
  ...
  const [show2FASetup, setShow2FASetup] = useState(false) // ✅ AGREGAR ESTA LÍNEA
  const [is2FAReset, setIs2FAReset] = useState(false) // ✅ AGREGAR ESTA LÍNEA
  const {
    products,
    movements,
    priceHistory,
  ...
  // ✅ FUNCIÓN PARA RECONFIGURAR 2FA - MODIFICADA
  const handle2FAReconfigure = () => {
    setShow2FASetup(true)
    setIs2FAReset(true) // ✅ AGREGAR ESTA LÍNEA
    setShowResetModal(true)
  }

  // ✅ FUNCIÓN PARA COMPLETAR SETUP 2FA - MODIFICADA
  const handle2FASetupComplete = (secret: string) => {
    setShow2FASetup(false)
    setIs2FAReset(false) // ✅ AGREGAR ESTA LÍNEA
    toast({
    ...
    switch (activeSection) {
      ...
      case "facturas-compra":
        return (
          <FacturaCompraForm 
            ...
            onUpdateProductPrice={async (productId: string, newPrice: number) => {
              try {
            ...
            onAddMovement={async (productId: string, quantity: number, reason: string) => {
              try {
      ...
      case "facturas-venta":
        return (
          <FacturaVentaForm 
            products={products}
            onSaleProduct={async (productId: string, quantity: number, reason: string) => {
              try {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-sm sm:text-lg font-bold text-red-600 truncate">ELECTROLUXSTORE</h1>
                  {currentUser && (
                    <div className="flex items-center gap-1 sm:gap-2 text-xs text-muted-foreground">
                      <User className="h-2.5 w-2.5 sm:h-3 sm:w-3 flex-shrink-0" />
                      <span className="truncate max-w-[120px] sm:max-w-none">{currentUser.username}</span>
                      {currentUser.has2FA && (
                        <Badge variant="outline" className="text-xs px-1 py-0 h-4 sm:h-auto sm:px-2 sm:py-1">
                  ...
                  <DropdownMenu open={showMobileMenu} onOpenChange={setShowMobileMenu}>
                    ...
                    >
                      ...
                      <div className="px-2 py-2">
                        <p className="text-xs font-medium text-muted-foreground mb-2 px-2">Navegación</p>
                        <div className="space-y-1">
                          <button
                            ...
                            className={`
                              ...
                              ${activeSection === 'products' ? 'bg-accent text-accent-foreground' : ''}
                            `}
                          >
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'add' ? 'bg-accent text-accent-foreground' : ''}
                              ${connectionStatus === "offline" ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                            `}
                          >
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'movements' ? 'bg-accent text-accent-foreground' : ''}
                            `}
                          >
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'alerts' ? 'bg-accent text-accent-foreground' : ''}
                            `}
                          >
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'prices' ? 'bg-accent text-accent-foreground' : ''}
                              ${connectionStatus === "offline" ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                            `}
                          >
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'reports' ? 'bg-accent text-accent-foreground' : ''}
                            `}
                          >
                      ...
                      {/* Sección de facturas */}
                      <div className="px-2 py-2">
                        <p className="text-xs font-medium text-muted-foreground mb-2 px-2">Facturación</p>
                        <div className="space-y-1">
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'facturas-compra' ? 'bg-accent text-accent-foreground' : ''}
                              ${connectionStatus === "offline" ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                            `}
                          >
                            <Building2 className="h-4 w-4 text-green-600 flex-shrink-0" />
                            <div className="text-left">
                              <div className="font-medium">Facturas de Compra</div>
                          ...
                          <button
                            ...
                            className={`
                              ...
                              hover:bg-accent hover:text-accent-foreground
                              focus:bg-accent focus:text-accent-foreground focus:outline-none
                              ${activeSection === 'facturas-venta' ? 'bg-accent text-accent-foreground' : ''}
                              ${connectionStatus === "offline" ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                            `}
                          >
                ...
                <CardContent className="p-3 sm:p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg sm:text-xl md:text-3xl font-bold text-red-600">{products.length}</p>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg sm:text-xl md:text-3xl font-bold text-green-600">${totalValue.toLocaleString()}</p>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg sm:text-xl md:text-3xl font-bold text-blue-600">{movements.length}</p>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={`text-lg sm:text-xl md:text-3xl font-bold ${lowStockAlerts.length > 0 ? 'text-red-600' : 'text-gray-600'}`}>
                        {lowStockAlerts.length}
                      </p>
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        {lowStockAlerts.length > 0 ? (
                    ...
                    <div className="relative">
                      <AlertTriangle className={`h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 ${lowStockAlerts.length > 0 ? 'text-red-600' : 'text-gray-400'}`} />
                      {lowStockAlerts.length > 0 && (
          ...
          {!isMobile && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-xl text-red-600">Herramientas de Gestión ELECTROLUXSTORE</CardTitle>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button 
                          className={`
                            ...
                            ${(activeSection === 'facturas-compra' || activeSection === 'facturas-venta') 
                              ? 'bg-background text-foreground shadow-sm' 
                              : 'hover:bg-background/50 hover:text-foreground'
                            }
                            ${connectionStatus === "offline" ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                        ...
                        >
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-xs text-muted-foreground">Compras y Ventas</div>
                          </div>
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent 
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            <div className="font-medium">Facturas de Compra</div>
                            <div className="text-xs text-muted-foreground">Registrar compras a proveedores</div>
                          </div>
                        </DropdownMenuItem>
                        <DropdownMenuItem 
                          onClick={() => handleNavigation('facturas-venta')}
                        ...
                        >
                          ...
                          <div>
                            <div className="font-medium">Facturas de Venta</div>
                            <div className="text-xs text-muted-foreground">Generar facturas a clientes</div>
                          </div>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
            ...
            <div className="space-y-4">
              ...
              {selectedProductForMovement && (
                <div className="p-3 bg-muted rounded-lg">
                  <p className="font-medium text-sm">{selectedProductForMovement.name}</p>
                  <p className="text-sm text-muted-foreground">
              ...
              <div className="flex flex-col gap-2 pt-4">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      {movementType === "entrada" ? "📦 Agregar al Stock" : "🛒 Registrar Venta"}
                    </>
                  )}
                </Button>
                <Button
