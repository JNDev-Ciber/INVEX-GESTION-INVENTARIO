...
export function ProductListTable({
  ...
  const handleSaveBarcode = async () => {
    ...
    // Permitir guardar vacío para eliminar
    const cleanedBarcode = barcodeInput.trim();

    // Solo validar formato si hay contenido
    if (cleanedBarcode && !/^\d{5,}$/.test(cleanedBarcode)) {
      toast({
    ...
    try {
      ...
      toast({
        title: cleanedBarcode ? "Código guardado" : "Código eliminado",
        description: cleanedBarcode || "El código de barras fue eliminado",
        duration: 2000,
      });
    ...
    <div className="space-y-4">
                ...
                {paginatedProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <div
                          className={`font-medium ${getMarginColor(margin)}`}
                        >
                          {margin.toFixed(1)}%
                        </div>
                        <div
      ...
      <Dialog open={barcodeDialogOpen} onOpenChange={setBarcodeDialogOpen}>
        <DialogContent className={isDark ? "bg-gray-900 border-gray-700" : ""}>
          <DialogHeader>
            ...
            <DialogDescription>
              Escanea o ingresa el código de barras para{" "}
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                {barcodeInput && (
                  <Button
                    ...
                    className={`${
                      isDark
                        ? "hover:bg-red-900/30 hover:text-red-300 hover:border-red-600"
            ...
            <div className="flex gap-2">
              ...
              {/* BOTÓN PARA ELIMINAR DE LA BASE DE DATOS */}
              {selectedProductForBarcode?.barcode && (
                <Button
                  variant="destructive"
                  onClick={async () => {
                    setIsSavingBarcode(true);
                    try {
                      await onBarcodeSave(selectedProductForBarcode.id, "");
                      toast({
                        title: "Código eliminado",
                        description: "El código de barras ha sido eliminado.",
                        duration: 2000,
                      });
                      setBarcodeDialogOpen(false);
                      setBarcodeInput("");
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm text-muted-foreground">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
          ...
          <span>
            Stock total:{" "}
            {filteredProducts.reduce((sum, p) => sum + p.quantity, 0)} unidades
          </span>
        </div>
      </div>
    </div>
